...
export default function Evaluaciones() {
  ...
  async function load() {
    try { const p = new URLSearchParams(); if (q) p.set('q', q); if (empresa) p.set('empresa', empresa); setItems(await api.get<V[]>(`/evaluaciones?${p}`)); }
  ...
  return (
    <>
      <h2 style={{ marginTop: 0 }}>Evaluaciones de desempeño</h2>
      ...
      <div className="card" style={{ padding: 0, overflow: 'auto' }}>
        <table>
          <thead><tr><th>Empleado</th><th>Empresa</th><th>Período</th><th>Tipo</th><th>Calificación</th><th>Comentarios</th></tr></thead>
          <tbody>
            {items.map((v) => (
              <tr key={v.id}><td>{v.nom} <span className="muted">({v.leg_num})</span></td><td>{v.empresa}</td><td>{v.periodo}</td><td>{v.tipo || '—'}</td><td>{v.calificacion || '—'}</td><td>{v.comentarios || '—'}</td></tr>
            ))}
            {!items.length && <tr><td colSpan={6} className="muted" style={{ textAlign: 'center', padding: 20 }}>Sin evaluaciones.</td></tr>}
          </tbody>
        </table>
      </div>
    </>
