...
export default function Sanciones() {
  ...
  async function load() {
    try { const p = new URLSearchParams(); if (q) p.set('q', q); if (empresa) p.set('empresa', empresa); setItems(await api.get<S[]>(`/sanciones?${p}`)); }
  ...
  return (
    <>
      <h2 style={{ marginTop: 0 }}>Sanciones</h2>
      ...
      <div className="card" style={{ padding: 0, overflow: 'auto' }}>
        <table>
          <thead><tr><th>Empleado</th><th>Empresa</th><th>Tipo</th><th>Fecha</th><th>Días</th><th>Descripción</th></tr></thead>
          <tbody>
            {items.map((s) => (
              <tr key={s.id}><td>{s.nom} <span className="muted">({s.leg_num})</span></td><td>{s.empresa}</td><td>{s.tipo}</td><td>{fmt(s.fecha)}</td><td>{s.dias || '—'}</td><td>{s.descripcion || '—'}</td></tr>
            ))}
            {!items.length && <tr><td colSpan={6} className="muted" style={{ textAlign: 'center', padding: 20 }}>Sin sanciones.</td></tr>}
          </tbody>
        </table>
      </div>
    </>
